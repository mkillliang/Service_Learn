...
public class APIController {
	...
	//搜索--------
	@RequestMapping(value="/article/s/{keyword}")
	public Page<Article> fingTextByKeyword(
			@PathVariable String keyword,
			@RequestParam(defaultValue="0") int page){
		return articleService.findTextByKeyword(keyword, page);
	}
	
	//Note 我的评论，别人评论自己
	@RequestMapping(value="/comments/article_id/receviedcomment/{page}")
	public Page<Comment> getCommentsToMyArticle(
			@PathVariable int page,
			HttpServletRequest request){
		User currentUser = getCurrentUser(request);
		int author_id = currentUser.getId();
		return commentService.findCommentsOfMe(author_id, page);
	}
	
	@RequestMapping(value="/comment/article_id/receviedcomment")
	public Page<Comment> getCommentsToMyArticle(HttpServletRequest request){
		User currentUser = getCurrentUser(request);
		int author_id = currentUser.getId();
		return commentService.findCommentsOfMe(author_id, 0);
	}
	
	@RequestMapping(value="/comments/my/receviedcomment/{page}")
	public Page<Comment> getMyCommentsOfArticle(
			@PathVariable int page,
			HttpServletRequest request){
		User currentUser = getCurrentUser(request);
		int author_id = currentUser.getId();
		return commentService.findMyComments(author_id, page);
	}
	
	@RequestMapping(value="/comment/my/receviedcomment")
	public Page<Comment> getMyCommentsOfArticle(HttpServletRequest request){
		User currentUser = getCurrentUser(request);
		int author_id = currentUser.getId();
