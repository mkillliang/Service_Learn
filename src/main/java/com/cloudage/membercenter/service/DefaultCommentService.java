...
public class DefaultCommentService implements ICommentService {
	...
	//别人的评论
	@Override
	public Page<Comment> findCommentsOfMe(int author_id, int page) {
		Sort sort = new Sort(Direction.DESC, "createDate");
		PageRequest pageReqeust = new PageRequest(page, 10, sort);
		return commentRepo.findCommentsOfMe(author_id, pageReqeust);
	}

	//我的评论
	@Override
	public Page<Comment> findMyComments(int author_id, int page) {
		Sort sort = new Sort(Direction.DESC, "createDate");
		PageRequest pageReqeust = new PageRequest(page, 10, sort);
		return commentRepo.findMyComments(author_id,pageReqeust);
	}
